...
# Visual Upgrades router will be included AFTER core utilities are defined to avoid circular imports.
...
async def upload_complete(request: Request, body: UploadComplete, db=Depends(get_db)):
    ...
    doc = {"id": str(uuid.uuid4()), "original_name": body.filename, "stored_as": final_name, "url": abs_url, "path": rel_path, "category": body.category or "general", "tags": body.tags, "uploaded_at": now_iso()}
...
async def whatsapp_webhook_receive(request: Request, db=Depends(get_db)):
    ...
    await db["whatsapp_events"].insert_one({"id": str(uuid.uuid4()), "received_at": now_iso(), "raw": payload, "source": "meta", "type": payload.get("object", "event")})
...
async def meta_send_text(to_norm: str, text: str, db: AsyncIOMotorDatabase) -> Dict[str, Any]:
    ...
    payload = {"messaging_product": "whatsapp", "recipient_type": "individual", "to": to_norm, "type": "text", "text": {"body": text or ""}}
...
async def whatsapp_send_template(body: WhatsAppSendTemplateRequest, db=Depends(get_db)):
    ...
    headers = {"Authorization": f"Bearer {META_ACCESS_TOKEN}", "Content-Type": "application/json"}
    payload: Dict[str, Any] = {"messaging_product": "whatsapp", "to": to_norm, "type": "template", "template": {"name": body.template_name, "language": {"code": body.language_code or "en"}}}
    if body.components:
