...
def test_endpoint(name, endpoint, method="POST", data=None, params=None):
    ...
    base_url = "https://aavana-crm.preview.emergentagent.com/api"
...
def main():
    ...
    try:
        response = requests.post(
            "https://aavana-crm.preview.emergentagent.com/api/leads",
            json=test_lead,
            headers={'Content-Type': 'application/json'},
            timeout=10
        )
    ...
    if test_lead_id:
        try:
            requests.delete(
                f"https://aavana-crm.preview.emergentagent.com/api/leads/{test_lead_id}",
                timeout=10
            )
